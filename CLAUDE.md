...
Four layers, low-level → UI. The key rule: the **protocol layer is pure and Node-testable**; only `device.ts`/`raster.ts` may touch browser/hardware APIs.
...
2. **Device transport (browser-only)** — `src/lib/displaypad/device.ts`, `raster.ts`
   `DisplayPad extends EventTarget`: `request()` / `fromGranted()` → `open()`; emits `keydown` / `keyup` / `close`. It drives the reverse-engineered transfer state machine (INIT → announce a key → pad echoes → stream pixel chunks) from inbound `inputreport` events. The pad is a **composite HID device with three interfaces** — see "Hardware caveats" below; `assignRoles()` splits the handles by report descriptor size, not enumeration order. WebHID delivers a report as `reportId` + `data`; `buildReport()` rejoins them so the protocol byte offsets line up (a no-op for the pad's unnumbered reports, where `reportId` is always 0 and `data` already starts at the real type byte). `raster.ts` turns an image URL into the 102×102 RGBA buffer the protocol expects. Verbose `[displaypad]` wire tracing is gated behind the `debug` singleton (below) and a checkbox in the UI.

3. **State (Svelte 5 runes singletons)** — `src/lib/state/*.svelte.ts`
   - `keymap` — the 12 `KeyConfig`s (label / face / action), persisted to `localStorage`. The source of truth for what each key looks like and does; hardware-agnostic.
   - `connection` — owns the single `DisplayPad`, mirrors pressed state into runes, pushes keymap faces to the hardware on explicit apply only (`applyKey` / `applyAll` — neither runs automatically on connect), and runs a key's configured action when it is pressed. On construction it silently tries `DisplayPad.fromGranted()` to reopen a previously-permitted pad without prompting; `connect()` (the `ConnectButton` click handler) is the only path that calls `DisplayPad.request()` and shows the picker.
